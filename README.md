# Sysafari 客户门户系统

客户门户是一个独立的 Web 应用，供 Sysafari 物流的客户查询订单、账单和管理 API 密钥。

## 系统架构

```
┌─────────────────────────────────────────────────────────────┐
│                    阿里云 ECS 服务器                         │
├─────────────────────────────────────────────────────────────┤
│  ┌───────────────┐  ┌───────────────┐  ┌───────────────┐   │
│  │ ERP 主系统    │  │ 演示环境      │  │ 客户门户      │   │
│  │ :3001         │  │ :3002         │  │ :3003         │   │
│  └───────────────┘  └───────────────┘  └───────────────┘   │
│          │                  │                  │            │
│          └──────────────────┼──────────────────┘            │
│                             │                               │
│                      ┌──────▼──────┐                        │
│                      │   Nginx     │                        │
│                      └──────┬──────┘                        │
└─────────────────────────────┼───────────────────────────────┘
                              │
              ┌───────────────┼───────────────┐
              │               │               │
    api.xianfeng-eu.com  demo.xxx   portal.xianfeng-eu.com
```

## 功能模块

| 模块 | 说明 |
|------|------|
| 🔐 认证 | 客户登录、退出、修改密码 |
| 📦 订单 | 订单列表、详情、物流跟踪 |
| 💰 账单 | 账单查询、PDF 下载、应付款 |
| 🔑 API 密钥 | 创建、管理、撤销 API 密钥 |

## 技术栈

**前端：**
- React 18 + TypeScript
- Tailwind CSS
- Vite
- Axios

**后端：**
- Node.js + Express
- PostgreSQL (portal_db)
- JWT 认证

## 本地开发

### 1. 安装依赖

```bash
# 前端依赖
npm install

# 后端依赖
cd server && npm install
```

### 2. 配置环境变量

```bash
# 复制环境变量模板
cp scripts/aliyun/env.local.example server/.env

# 编辑配置
vim server/.env
```

### 3. 创建数据库

```bash
# 连接本地 PostgreSQL
psql -U postgres

# 创建数据库
CREATE DATABASE portal_db;

# 初始化表结构
\c portal_db
\i scripts/sql/init-portal-db.sql
```

### 4. 启动服务

```bash
# 终端 1：启动后端 (端口 3003)
cd server && npm run dev

# 终端 2：启动前端 (端口 5174)
npm run dev
```

### 5. 访问

- 前端：http://localhost:5174
- API：http://localhost:3003/api/health

## 部署

详见 [部署指南](./scripts/aliyun/README.md)

## 目录结构

```
sysafari-customer-portal/
├── src/                    # 前端源码
│   ├── components/         # 组件
│   ├── contexts/           # Context
│   ├── pages/              # 页面
│   └── utils/              # 工具函数
├── server/                 # 后端源码
│   ├── config/             # 配置
│   ├── middleware/         # 中间件
│   ├── modules/            # 业务模块
│   │   ├── auth/           # 认证
│   │   ├── order/          # 订单
│   │   ├── finance/        # 财务
│   │   └── api-keys/       # API 密钥
│   └── app.js              # 入口文件
├── scripts/                # 脚本
│   ├── sql/                # SQL 脚本
│   └── aliyun/             # 阿里云部署脚本
├── dist/                   # 构建输出
└── package.json
```

## API 文档

### 认证

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | /api/auth/login | 登录 |
| GET | /api/auth/me | 获取当前用户 |
| POST | /api/auth/change-password | 修改密码 |
| POST | /api/auth/logout | 退出登录 |

### 订单

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/orders | 订单列表 |
| GET | /api/orders/:id | 订单详情 |
| GET | /api/orders/:id/tracking | 物流跟踪 |
| GET | /api/orders/stats/summary | 订单统计 |

### 财务

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/finance/invoices | 账单列表 |
| GET | /api/finance/invoices/:id | 账单详情 |
| GET | /api/finance/invoices/:id/download | 下载 PDF |
| GET | /api/finance/payables/summary | 应付款汇总 |

### API 密钥

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | /api/api-keys | 密钥列表 |
| POST | /api/api-keys | 创建密钥 |
| PUT | /api/api-keys/:id | 更新密钥 |
| DELETE | /api/api-keys/:id | 撤销密钥 |

## 许可证

MIT License
